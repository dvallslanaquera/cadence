...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (response.status === 401) {
    // Stale tab: session went away while the page stayed open. Skip the redirect on /login, or the login page's own settings fetch would 401 and reload /login onto itself forever.
